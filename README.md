...
Тесты покрывают основную функциональность конструктора бургеров и раздела «Лента заказов», построены по паттерну Page Object, запускаются в `Google Chrome` и `Mozilla Firefox`, а результаты выполнения оформляются в подробный `Allure`-отчёт на русском языке.
